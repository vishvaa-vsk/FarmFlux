/// The main entry point of the FarmFlux application.
///
/// This file initializes Firebase, loads environment variables, and sets up
/// the `MultiProvider` for state management. It also defines the `FarmFluxApp`
...
final GlobalKey<State<FarmFluxApp>> farmFluxAppKey =
    GlobalKey<State<FarmFluxApp>>();
...
void main() async {
  ...
  // Load dotenv first, then initialize Firebase
  await StartupPerformance.measure('firebase_dotenv_sequential', () async {
    // Load environment variables first
    await dotenv.load(fileName: ".env").timeout(const Duration(seconds: 2));

    // Then initialize Firebase with the loaded environment variables
  ...
  runApp(
    MultiProvider(
      ...
      child: FarmFluxApp(key: farmFluxAppKey),
    ),
  );
...
class FarmFluxApp extends StatefulWidget {
  const FarmFluxApp({super.key});

  @override
  State<FarmFluxApp> createState() => _FarmFluxAppState();
}

class _FarmFluxAppState extends State<FarmFluxApp> {
  Locale? _locale;
